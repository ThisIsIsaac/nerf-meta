...
def test(args):
    ...
    for idx, (imgs, poses, hwf, bound) in enumerate(test_loader):
        ...
        vid_frames = create_360_video(args, model, hwf, bound, device, idx+1, savedir)
        wandb.save(str(savedir))
        wandb.log({"test/vid": wandb.Video(vid_frames.transpose(0, 3, 1, 2), fps=30, format="mp4")})
        
        print(f"scene {idx+1}, psnr:{scene_psnr:.3f}, video created")
        wandb.log({"test/scene_psnr": scene_psnr})
