...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        x_proj = x.unsqueeze(dim=-2) * self.freqs.unsqueeze(
            dim=-1)  # (num_rays, num_samples, num_freqs, in_features)
        x_proj = x_proj.reshape(*x.shape[:-1],
                                -1)  # (num_rays, num_samples,
        # num_freqs*in_features)
...
def build_nerf(args):
    model = SimpleNeRF(in_features=3, max_freq=args.max_freq,
